...
\begin{description}
	\item[HTML5] refers to a set of semantic tag (like \ctag{footer},
...
the DOM by prefixing the standard method (i.e. \linebreak\code{requestAnimFrame}
can became \code{mozRequestAnimFrame} or \code{webkitRequestAnimFrame})
with a browser specific prefix\footnote{\code{o}: for
Opera, \code{ms}: for Internet Explorer, \code{moz}: for Firefox, and
\code{webkit}: for the WebKit based browser (Chrome and Safari)}. This prefixing
...
one can \emph{paint} anithing. On top of this, the \code{canvas} element give
...
\citetitle{socket} provide a single entry-point to create a connection to the
...
leaverage on the \code{setTimeout} function of \js{} in order to split code into
...
\ac{HTML}5 support a number of storage techniques able to store data within the
...
\begin{description}
	...
	key-value pairs like any \js{} \code{Object} stored in the browser accessible
...
In this category falls \code{application cache}. The \code{application cache} is
